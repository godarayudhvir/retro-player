...
- **Compact Mobile Dropdown Fallback (`.mobile-sub-toolbar`)**: On small mobile devices (<= 768px), provides a collapsible hamburger menu button (`<Menu size={18} />`) that slides down the full action suite with 100% desktop feature parity (Restart, Pause, Mute, Record, Speed, Capture, Shader/Pixel, Quick Save, Quick Load, and Diagnostics) with horizontal touch scrolling and zero viewport overflow.
...
- **60 FPS Screen Video Recording Engine with Audio Mirror & EBML Patcher**: Native `MediaRecorder` integration capturing active gameplay canvas and Web Audio sound tracks into `.webm` video files with live topbar indicator, automatic audio node mirroring (`attachRecordingDestination`), client-side EBML `Duration` header patching (`fixWebmDuration`) to prevent broken playback/seeking, and automatic download.

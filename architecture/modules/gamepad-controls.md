...
- **UI Shell Navigation** (when `activeGame` is null):
  ...
  - `Complete Modal D-Pad Nav`: In-app Game Detail Drawer (`play`, `fav`, `editMeta`, `scrape`, `close`), Profile Select Modal (`profile cards`, `add profile`, `manage`, `close`), Multiavatar Profile Creator Studio (`close`, `nameInput`, `seedInput`, `random`, preset cards, color swatches, `save`, `cancel`), and System Settings Hub (Back button, left category sidebar with D-Pad `UP`/`DOWN`, pressing `RIGHT` or `A` enters the settings detail pane to navigate interactive settings controls/inputs/buttons, pressing `LEFT` returns to sidebar, and `B` exits) support 100% keyboard and gamepad spatial movement.

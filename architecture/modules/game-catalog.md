...
- **12-Console ROM Directory Scanning**: Recursively scans `/public/roms` (or `ROMS_DIR`) for valid extensions across all 12 supported retro systems:
  ...
  - Arcade MAME (`.zip`)
  - Atari 2600 (`.a26`)
- **Bundled Non-Commercial Demo Showcase (52 Titles Across 12 Consoles)**: Pre-packages a curated roster of 52 non-complete demonstration ROMs, trade show samples, Sega Channel trials, aftermarket homebrew, and prototypes in `public/roms/` across all 12 supported platforms (3–5 per system, documented in detail in [roms.md](../../roms.md)). Titles are specifically non-complete slices curated to evaluate WebAssembly core emulation performance with zero full commercial retail releases.
- **Optimized Distribution Architecture**:
  - **Curated Web Demo Tier (52 Titles — 510 MB)**: Whitelisted and tracked in Git across all 12 consoles (`arcade`, `atari_2600`, `game_gear`, `gb`, `gba`, `gbc`, `n64`, `nes`, `playstation`, `nds`, `sega_genesis`, `snes`), with zero heavy bloat or unnecessary ROM duplicates, fully compliant with GitHub push limits and GitHub Pages quotas.

...
- **Built-in 60 FPS Lossless Screen Recorder**: Native `MediaRecorder` integration capturing active gameplay directly from canvas (`canvas.captureStream(60)`) mixed with in-game audio tracks into crystal-clear `.webm` recordings, complete with live topbar `REC ● MM:SS` timer badge and one-click auto-download.

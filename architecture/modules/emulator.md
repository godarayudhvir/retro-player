...
- **Unified In-Game Topbar & Action Dispatch**: Direct topbar action suite hosting instant Restart, Pause/Resume, Mute/Unmute audio toggling, 60 FPS Screen Recording, Fast-Forward Emulation Speed cycling (1.0x - 5.0x), Lossless Screenshot Capture (PNG), 1-Click Display Shader Cycling, Quick Save, and Quick Load state actions directly in the main header bar without obstructing popup panels on desktop displays.
- **Unmuted-by-Default Audio Architecture & Proactive Synchronization**: Guarantees all games start 100% unmuted (`window.EJS_volume = 1.0`, `volume = 1.0`, `isGameMuted = false`), purging any lingering `ejs_muted` storage state from prior sessions upon bootstrap. Automatically unlocks and resumes suspended Web Audio API / OpenAL contexts (`audioCtx.resume()`) across `EJS_ready`, `EJS_onGameStart`, stage focus, and host interaction listeners, preventing audio/UI desync.
- **Built-in 60 FPS Lossless Screen Recorder with EBML Duration Patcher**: Native `MediaRecorder` integration capturing active gameplay directly from canvas (`canvas.captureStream(60)`) synchronized with audio mirrored from WebAudio/OpenAL/SDL audio nodes via `attachRecordingDestination` into `.webm` recordings. Includes an inline EBML header patcher (`fixWebmDuration`) to inject accurate duration metadata (avoiding `00:01` / broken seek issues in media players), complete with live topbar `REC ● MM:SS` timer badge and one-click auto-download.
- **Configurable Emulation Speed (1.0x - 5.0x)**: Live hardware speed adjustments supporting `1.0x Normal`, `1.5x`, `2.0x Turbo`, `3.0x`, `4.0x`, and `5.0x Max Boost` with instant topbar button cycling.

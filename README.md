...
- 🎨 **Multi-Theme Console Studio (Vanilla & DS Touch with Light/Dark Modes)**: Switch seamlessly between **Vanilla** (porcelain white / slate dark 3D cartridge shelf) and **DS Touch** (Nintendo DS dual-screen firmware with direct integrated action stage & graph paper styling) with persistent Light/Dark mode toggle, official platform vector badges, and full gamepad/keyboard navigation (`T` hotkey).

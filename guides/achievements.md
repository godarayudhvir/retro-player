...
> **Status Checklist**: Use the `Status / Working` column to mark off testing verification on your local setup (`[x]` Working, `[-]` In Progress, `[ ]` Needs Testing).
...
| :---: | :---: | :---: | :--- | :--- | :--- | :--- | :---: |
| [x] | 🥉 Bronze | +10G | `first_launch` | **Insert Coin** | Launch your very first retro game in Retro Player. | `totalLaunches >= 1` | Global & Per-ROM |
| [x] | 🥉 Bronze | +10G | `console_hopper` | **Console Hopper** | Play at least one game across 3 distinct retro systems. | Unique system keys played `>= 3` | Global |
| [x] | 🥈 Silver | +25G | `gen_traveler` | **Generation Traveler** | Play games across 8-bit, 16-bit, and 32/64-bit platforms. | Played `[NES/GB/GG]` + `[SNES/MD]` + `[N64/PS1/NDS/GBA]` | Global |
...
| [x] | 🥉 Bronze | +10G | `indecisive_swapper` | **Indecisive Swapper** | Launch 3 different games within 3 minutes. | 3 distinct game boots within `180s` | Global |
| [x] | 🥉 Bronze | +10G | `library_tourist` | **Library Tourist** | Launch 5 different games in a single active session. | Single session launch list `>= 5` | Global |
| [x] | 🥈 Silver | +25G | `cartridge_collector` | **Cartridge Collector** | Have at least 25 ROMs indexed in your local library. | Mounted `games.length >= 25` | Global |
| [x] | 🥇 Gold | +50G | `grand_archivist` | **Grand Archivist** | Have 100+ ROMs indexed in your library across all consoles. | Mounted `games.length >= 100` | Global |
...
| [ ] | 🥇 Gold | +50G | `weekly_streak_7` | **Relentless Habit** | Play games for 7 consecutive days in a row. | Consecutive active calendar days `>= 7` | Global |
...
| Status | Tier | Points | ID | Title | Universal Requirement | Evaluation Logic | Scope |
| :---: | :---: | :---: | :--- | :--- | :--- | :--- | :---: |
| [x] | 🥉 Bronze | +10G | `rage_quit` | **Instant Regret?** | Exit a game in under 45 seconds of launching. | Session duration `3s < s < 45s` | Global & Per-ROM |
...
| [ ] | 🥉 Bronze | +10G | `retro_purist` | **Retro Purist** | Toggle ON the authentic CRT scanline shader during gameplay. | CRT filter enabled | Global |
| [ ] | 🥈 Silver | +25G | `certified_tactile`| **Certified Tactile** | Play any game using a real connected physical gamepad controller. | Physical gamepad button/axis input | Global |
...
| Status | Tier | Points | ID | Title | Universal Requirement | Evaluation Logic | Scope |
| :---: | :---: | :---: | :--- | :--- | :--- | :--- | :---: |
| [ ] | 🥉 Bronze | +10G | `strategy_scholar`| **Strategy Scholar** | Spend over 60 seconds reading a game's Strategy Guide in the library. | Strategy guide open duration `>= 60s` | Global & Per-ROM |
| [ ] | 🥉 Bronze | +10G | `cover_connoisseur`| **Cover Connoisseur** | Scrape or edit custom 3D box artwork for any game. | Scraper / metadata update | Global & Per-ROM |

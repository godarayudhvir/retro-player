...
| `INCLUDE_DEMO_ROMS` | `true` | Set to `true` to include bundled showcase demo ROMs (29 titles); set to `false` for purely your own library. |
...
- **UI Experience**: Instant access to all 29 curated showcase demo ROMs and chiptune BGM tracks. Any custom games you drop in `./roms/` are automatically merged into your library.
...
- **Host Disk Impact**: On first startup, Docker copies the 29 demo ROM files and BGM tracks directly onto your host disk inside `./roms/` and `./bgm/`, allowing you to inspect, organize, or edit the raw files locally.

/**
 * Dedicated Pokémon Ruby Version Save Parser (GBA - 64 KB / 128 KB Flash).
...
export function parsePokemonRuby(data) {
  const activeSections = getActiveSections(data);
  if (!activeSections) return null;

  // Section 0: Trainer Info (Ruby security key offset 0x0AC4)
  let money = 0;
  ...
  // Section 1: Team & Party (Ruby party at sec1 + 0x0234)
  let partyCount = 0;
  let maxPartyLevel = 0;
  let hasShiny = false;
  ...
  return {
    ...
    events: {
      ...
      teamMagma: badgeCount >= 7,
      abandonedShip: badgeCount >= 5
    },
    legendaries: {
      groudon: hasLegendary || badgeCount >= 8,

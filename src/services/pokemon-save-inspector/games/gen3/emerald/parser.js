/**
 * Dedicated Pokémon Emerald Version Save Parser (GBA - 64 KB / 128 KB Flash).
...
export function parsePokemonEmerald(data) {
  const activeSections = getActiveSections(data);
  if (!activeSections) return null;

  // Section 0: Trainer Info (Emerald security key offset 0x0AC4)
  let money = 0;
  ...
  if (sec0 !== undefined) {
    ...
    const securityKey = readUint32LE(data, sec0 + 0x0AC4);
  ...
  // Section 1: Team & Party (Emerald party at sec1 + 0x0234)
  let partyCount = 0;
  let maxPartyLevel = 0;
  let hasShiny = false;
  ...
  if (sec1 !== undefined) {
    const rawPartyCount = data[sec1 + 0x0234] || 0;
    partyCount = (rawPartyCount >= 1 && rawPartyCount <= 6) ? rawPartyCount : 0;
    const partyStart = sec1 + 0x0238;
  ...
  return {
    ...
    events: {
      ...
      teamMagmaAqua: badgeCount >= 7,
      mirageTower: badgeCount >= 4,
      abandonedShip: badgeCount >= 5,
      cataclysmAwakening: badgeCount >= 7,
      emeraldRayquazaSoothe: badgeCount >= 8,
      battleFrontier: badgeCount >= 8
    },
    legendaries: {
      weatherTrio: hasLegendary || badgeCount >= 8,
